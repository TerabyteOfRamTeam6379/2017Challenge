...
public class Robot extends IterativeRobot {
	...
	public void autonomousPeriodic() {
		// Drive for 2 seconds
		if (timer.get() < 2.0) { // Change to distance travelled/total wheel rotations
			 // drive forwards half speed
		} else {
			 // stop robot
		}
		
	/*public void autonomous Periodic() {
	 * String gameData;
		gameData = DriverStation.getInstance().getGameSpecificMessage();
	 *	if(distanceTravelled < distance to closest switch/lever) {
	 *  	drive X inches/feet
	 *  	}
		if(gameData.charAt(0/1) == 'L') {
			travel to closest side of the left half of the switch/scale 
		} else {
			travel to closest side of the left right of the switch/scale
			}
	 *  drop off powerCube on switch/scale
	 *  }
	 */
	/*public void autonomous Periodic() {
	 * String gameData;
